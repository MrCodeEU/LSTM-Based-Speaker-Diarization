...
hp = Hyperparameters()
model_path = "best_model_06-12-2024_11-46-16.pth"
if hp.eval_dataset == "data_conv":
    ...
    for sigma in np.arange(0, 6.0, 0.5):
        for percentile in np.arange(80, 99, 3):
            ...
            for file in audio_files:
                ...
                _, der = run_diarization(file, model_path, seed=50, sigma=sigma, percentile=percentile)
                total_der += der
    ...
    for sigma in tqdm(np.arange(0, 6.0, 0.5), desc="Optimizing Sigma", unit="sigma"):
        for percentile in np.arange(80, 99, 3):
            total_der = 0.0
            num_files_processed = 0
            ...
            for i in range(0, 2):
                ...
                _, der = run_diarization(audio_path, model_path, callhome_index=n)
                total_der += der
                num_files_processed += 1
